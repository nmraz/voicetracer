...
class Stream(object):
    ...
    def __init__(self, str, pos = 0, start = 0):
        '''Initializes the stream with `str` and sets its token start and end to 0'''
        self.str = str
        self.pos = pos
        self.start = start
    ...
    def eat(self, chs):
        '''If the next character is one of `chs`, advances the stream and returns True; returns
        False otherwise
        '''
    ...
    def eat_while(self, chs):
        '''Advances the stream until a character which isn't one of `chs` is encountered'''
        while self.eat(chs):
            pass
    ...
    def newtok(self):
        '''Skips any whitespace and sets the token start to the current position, starting a
        new token
        '''
        self.eat_while('\s')
        self.start = self.pos
...
class Val(object):
    '''An object which may be used as either a string, a dictionary, or a list.
    These qualities make it easy to format the object into format strings expecting objects with
    various characteristics (e.g., expects a key `sa_family`, which may or may not exist)
    '''
    ...
    def __str__(self):
        '''Behave like a string (simply return string representation of underlying object)'''
...
# The following functions implement a recursive-descent parser for the c-like syntax which strace
# uses, with one caveat: expressions are not parsed fully. This may, on rare occasions, lead to
# misinterpretation of the string, although it is much better than having no parser at all.

def eat_string(stream):
    '''Advances the stream to the end of the string, respecting escape sequences'''
    escaped = False  # are we in an escape sequence?
    ch = stream.next()
    while ch is not None:
        if ch == '"' and not escaped: break  # end of string
        escaped = ch == '\\' and not escaped  # every backslash toggles "escapiness"
        ch = stream.next()

def parse_array(stream, term):
    '''Parses an array terminated by `term`, and returns a list of the elements'''
    # for example, to parse '[a, b, c]',
    # advance past the first '[' and call parse_array(stream, ']')
    # note: parse_array will see this as 'a, b, c]'
    ret = []
    if not stream.eat_tok(term):  # non-emptiness check
        ret.append(parse_expr(stream, ',' + term))  # use ',' and term as sync tokens
...
def parse_struct_field(stream):
    '''Parses something of the form 'name=expr' and returns a tuple (name, expr) or None on failure'''
    stream.newtok()
...
def parse_struct(stream):
    ...
    if not stream.eat_tok('}'):  # non-emptiness check
        name, expr = parse_struct_field(stream)  # this is definitely safe, see parse_expr
        ret[name] = expr
...
def parse_expr(stream, terms):
    '''Attempts to parse an expression, and jumps to one of `terms` at the end/at failure.
    Returns a Val representing what it parsed (e.g. a struct, array, string)
    '''
    ret = Val()
    stream.newtok()
    if stream.eat('"'):  # start of string
        ...
        if parse_struct_field(tmp_stream):
            ...
            ret = Val(parse_array(stream, '}'))  # this can happen in select, for example
    else:
...
def parse_sys_call(stream):
    '''Parses to the end of the system call, and returns (args, ret, err), where args is a list of
    the arguments, ret is the return value, and err is the description following it
    '''
